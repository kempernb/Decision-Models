...
## ver. 0.3, 04/03/2021, C.Sox : add constraint sets for integer variables
    ...
    for var in var_lst:
        ...
        if var.integer() == True:
            if lower == -infinity or upper == infinity:
                print('Integer variable',var.name(), 'does not have finite bounds')
                break
            print(str(var) + ' in', list(range(int(lower),int(upper)+1)))
        else:
